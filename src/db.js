...
async function addRegSensor4(arr, sensor_type) {
  ...
  const registered_date = arr[1];
  const value = arr[3];
  const encrypted = arr[5]
  const unit = arr[4];
  const insertQuery = `WITH sensor_data AS (SELECT s.id AS sensor_id FROM sensor s INNER JOIN station st ON s.station = st.id WHERE st.serial = $1 AND s.name = $2 LIMIT 1) INSERT INTO reg_sensor_${sensor_type} (registered_date, sensor, value, unit, encrypted_data) SELECT $3, sensor_id, $4, $5, $6 FROM sensor_data;`;
  try {
    const res = await client.query(insertQuery, [serial,name,registered_date,value,unit, encrypted]);
    if (res.rowCount > 0) {
      console.log("Registro del sensor '" + arr[2] + "' de la estación " + arr[0] + " insertado con éxito.");
      utils.logWrite("Registro del sensor '" + arr[2] + "' de la estación " + arr[0] + " insertado con éxito", "databaseLogs");
    } else {
